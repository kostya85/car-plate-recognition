...
from yolov3.utils import image_preprocess, read_class_names
...
class Dataset(object):
    ...
    def __init__(self, dataset_type, TEST_INPUT_SIZE=TEST_INPUT_SIZE):
        self.annot_path = (
            TRAIN_ANNOT_PATH if dataset_type == "train" else TEST_ANNOT_PATH
        )
        self.input_sizes = (
            TRAIN_INPUT_SIZE if dataset_type == "train" else TEST_INPUT_SIZE
        )
        self.batch_size = (
            TRAIN_BATCH_SIZE if dataset_type == "train" else TEST_BATCH_SIZE
        )
    ...
    def Delete_bad_annotation(self, bad_annotation):
        ...
        bad_image_name = bad_annotation[0].split("/")[
            -1
        ]  # can be used to delete bad image
        bad_xml_path = (
            bad_annotation[0][:-3] + "xml"
        )  # can be used to delete bad xml file
            ...
            if self.batch_count < self.num_batchs:
                while num < self.batch_size:
                    ...
                    try:
                        ...
                        print(
                            "IndexError, something wrong with",
                            annotation[0],
                ...
                if exceptions:
                    print("\n")
                    raise Exception(
                        "There were problems with dataset, I fixed them, now restart the training process."
                    )
                self.batch_count += 1
                if not self.train_yolo_tiny:
    ...
    def parse_annotation(self, annotation, mAP="False"):
        ...
        image, bboxes = image_preprocess(
            np.copy(image), [self.input_sizes, self.input_sizes], np.copy(bboxes)
        )
        return image, bboxes
    ...
    def preprocess_true_boxes(self, bboxes):
        ...
        bboxes_xywh = [
            np.zeros((self.max_bbox_per_scale, 4)) for _ in range(OUTPUT_LEVELS)
        ]
