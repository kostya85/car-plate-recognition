...
from yolov3.utils import (detect_image, image_preprocess, load_yolo_weights,
                          nms, postprocess_boxes, read_class_names)
...
def get_mAP(
    ...
    for index in range(dataset.num_samples):
        ...
        image = image_preprocess(
            np.copy(original_image), [TEST_INPUT_SIZE, TEST_INPUT_SIZE]
        )
        ...
        bboxes = postprocess_boxes(
            pred_bbox, original_image, TEST_INPUT_SIZE, score_threshold
        )
...
if __name__ == "__main__":
    if YOLO_FRAMEWORK == "tf":  # TensorFlow detection
        ...
        saved_model_loaded = tf.saved_model.load(
            f"./checkpoints/{TRAIN_MODEL_NAME}", tags=[tag_constants.SERVING]
        )
